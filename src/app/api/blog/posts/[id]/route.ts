...
async function ensureTags(keys: string[]) {
  ...
  for (const k of keys ?? []) {
    ...
    if (!tag)
      tag = await prisma.etiqueta.create({ data: { nombre: k, slug: s } });
    ids.push(tag.id);
  }
  return ids;
}
...
export async function PUT(
  ...
  const user = await getUserFromCookie(); // 👈 await here
  ...
  const minutosLectura =
    ...
    Math.max(
      1,
      Math.round(
        (data.contenidoMd ?? "").split(/\s+/).filter(Boolean).length / 220
      )
    );
  ...
  try {
    ...
    if (isKnown(e) && e.code === "P2002")
      return new NextResponse("Slug ya existe", { status: 409 });
...
/**
 * DELETE /api/posts/:id
 * Soft-deletes a post by setting estado_borrado = true
 */
...
/**
 * PATCH /api/posts/:id
 * Reactivates or toggles by accepting { estado_borrado: boolean } in body.
 * If body is absent, defaults to re-activate (false).
 */
export async function PATCH(req: NextRequest, { params }: { params: unknown }) {
  try {
    const user = await getUserFromCookie();
    if (!hasRole(user, "Admin")) {
      return new NextResponse("No autorizado", { status: 401 });
    ...
    const bodySchema = z
      .object({
        estado_borrado: z.boolean().optional(),
      })
      .optional();
    ...
    const nextValue =
      typeof parsed.estado_borrado === "boolean"
        ? parsed.estado_borrado
        : false;
